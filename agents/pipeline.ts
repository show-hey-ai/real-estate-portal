 ...
 */
import { query, type AgentMcpServerSpec } from '@anthropic-ai/claude-agent-sdk'
...
function toAgentMcpServers(
  mcpServers?: Record<string, { command: string; args: string[] }>
): AgentMcpServerSpec[] | undefined {
  return mcpServers ? [mcpServers] : undefined
}
...
export async function runPipeline() {
  ...
  const orchestratorPrompt = `あなたは宿泊業物件専門ポータル「Ziyou Hospitality」の物件登録パイプラインのオーケストレーターです。
    ...
    options: {
      ...
      agents: {
        'reins-scraper': {
          ...
          mcpServers: toAgentMcpServers(reinsScraper.mcpServers),
        },
        'pdf-extractor': {
